...
name       = 'ELVIS'
surname    = 'PALOS'
student_id = '1980480'    # your Sapienza registration number
...
def func1(dict1, dict2):
    ...
    # Find the common keys by using the intersection method of sets
    common_keys = set(dict1.keys()).intersection(set(dict2.keys()))

    for key in common_keys:
        # Get the values from both dictionaries
        list1 = dict1[key]
        list2 = dict2[key]

        # Find unique values in each list
        unique_values = set(list1).symmetric_difference(set(list2))

        # Sort unique values first by decreasing length, then by ascending alphabetical order
        sorted_values = sorted(unique_values, key=lambda x: (-len(x), x))

        # Add the sorted values to the result dictionary
...
def func2(text):
    ...
    text = text.lower()
    # Split the text into words. This method automatically creates a list to store the stuff
    words = text.split()
    
     # Initialize the dictionary to keep counts of initial letters
    letter_count = {}
    ...
    for word in words:
        ...
        # Update the dictionary count for this initial letter
