...
def func1(a_dict, word):
  ...
  #   if word in val:
  #     a_dict.pop(key) # if we don't give a fuck about the value returned by .pop(), we can use del
  #     count += 1
  # return count

  # keys_to_remove = [key for key, value in a_dict.items() if word in value]
  # for key in keys_to_remove:
  #     a_dict.pop(key)
  # return len(keys_to_remove)
  
  dict_2 = a_dict.copy()
  num = [a_dict.pop(key) for key, val in dict_2.items() if word in val]
  return len(num)
...
def func2(a_string):
    # return ''.join(sorted(set(a_string), reverse=True)) # Pythonic way
    
    # Convert the string to a set to get unique characters
    unique_chars = set(a_string)
    # Sort the unique characters in reverse alphabetical order
...
# key Function: A function that will be called on each element in the list to generate a comparison key.
# lambda x: ...: An anonymous function (lambda function) that takes an element x as input and returns a tuple (len(x), x).

""" The tuple (len(x), x) means that the elements will be sorted primarily by their length (len(x)) and then alphabetically
(or numerically, depending on the type of x) if there are elements of the same length.  """
